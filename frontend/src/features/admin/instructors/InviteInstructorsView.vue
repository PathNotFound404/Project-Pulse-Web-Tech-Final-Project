<script setup>
import { ref } from 'vue'
import { inviteInstructors } from './instructorAdminService.js'
...
async function handleInvite() {
  ...
  loading.value = true
  try {
    links.value = await inviteInstructors(emails)
  } catch (e) {
...
<template>
  <div>
    ...
    <h1>Invite Instructors</h1>
    <p class="hint">Enter one email address per line (or comma-separated). Each will receive a unique registration link.</p>

    <ErrorMessage :message="error" />

    <textarea v-model="emailsText" rows="6" placeholder="instructor1@tcu.edu&#10;instructor2@tcu.edu" class="email-input" />
    ...
    <div v-if="links.length" class="results">
      ...
      <p class="hint">Share these links with the corresponding instructors.</p>
